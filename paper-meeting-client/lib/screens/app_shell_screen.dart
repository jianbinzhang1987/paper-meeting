            ...
            child: MeetingSurfaceCard(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('正在同步会议数据', style: Theme.of(context).textTheme.headlineSmall),
...
class _ConfigurationScreenState extends State<ConfigurationScreen> {
  ...
  void didChangeDependencies() {
    ...
    portController = TextEditingController(text: controller.config.port.toString());
    ...
    deviceController = TextEditingController(text: controller.config.deviceName);
    initialized = true;
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ConstrainedBox(
                          ...
                          child: Text(
                            ...
                            style: Theme.of(context).textTheme.bodyLarge?.copyWith(
                                  color: palette.textOnChrome.withValues(alpha: 0.8),
                                ),
                          ),
                        ),
                        ...
                        Wrap(
                          ...
                          runSpacing: 12,
                          children: [
                            _GlassChip(label: '服务器', value: '${controller.config.serverIp}:${controller.config.port}'),
                            _GlassChip(label: '会议室', value: controller.config.roomName),
                            _GlassChip(label: '座位', value: controller.config.seatName),
                            _GlassChip(label: '设备', value: controller.config.deviceName),
                          ],
                        ),
                    ...
                    child: ConstrainedBox(
                      ...
                      child: MeetingSurfaceCard(
                        ...
                        child: Column(
                          ...
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('设备绑定', style: Theme.of(context).textTheme.headlineSmall),
                            const SizedBox(height: 8),
                            Text(
                              '完成终端基础配置后进入签到页。',
                            ...
                            Row(
                              children: [
                                ...
                                SizedBox(width: 140, child: _field('端口', portController)),
                              ],
                            ),
                            const SizedBox(height: 12),
                            Row(
                              children: [
                                ...
                                const SizedBox(width: 12),
                                SizedBox(width: 140, child: _field('座位号', seatController)),
                              ],
                            ),
                            const SizedBox(height: 12),
                              ...
                              child: MeetingActionButton(
                                onPressed: () {
                                  controller.completeConfiguration(
                                    ...
                                    port: int.tryParse(portController.text.trim()) ?? 9394,
  ...
  Widget _field(String label, TextEditingController controller, {double width = 200}) {
    return SizedBox(
      width: width,
      child: TextField(
...
class _SignInScreenState extends State<SignInScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final seatMatchedUsers = controller.session.users.where((user) => user.seatName == controller.config.seatName).toList();
    final visibleUsers = seatFilterOnly && seatMatchedUsers.isNotEmpty ? seatMatchedUsers : controller.session.users;
    ...
    if (selectedUser != null && !visibleUsers.any((user) => user.id == selectedUser!.id)) {
      selectedUser = null;
    }
    final palette = context.meetingPalette;
    return Scaffold(
                      ...
                      children: [
                        ...
                        ConstrainedBox(
                          ...
                          child: Text(
                            ...
                            style: Theme.of(context).textTheme.bodyLarge?.copyWith(
                                  color: palette.textOnChrome.withValues(alpha: 0.82),
                                ),
                          ),
                        ),
                        const SizedBox(height: 28),
                        _GlassChip(label: '会议室', value: controller.config.roomName, block: true),
                        _GlassChip(label: '座位', value: controller.config.seatName, block: true),
                        _GlassChip(label: '服务器', value: '${controller.config.serverIp}:${controller.config.port}', block: true),
                      ],
                    ),
                  ),
                ),
                    ...
                    child: ConstrainedBox(
                      ...
                      child: MeetingSurfaceCard(
                        ...
                        child: Column(
                          ...
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('参会签到', style: Theme.of(context).textTheme.headlineSmall),
                            const SizedBox(height: 8),
                            Text(
                              '选择账号并完成身份确认后进入会议工作台。',
                              style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: palette.textSecondary),
                            ...
                            if (seatMatchedUsers.isNotEmpty) ...[
                              ...
                              SwitchListTile(
                                ...
                                subtitle: Text('当前座位 ${controller.config.seatName} 已匹配 ${seatMatchedUsers.length} 个账号'),
                                onChanged: (value) => setState(() => seatFilterOnly = value),
                              ),
                            ],
                            const SizedBox(height: 24),
                            DropdownButtonFormField<MeetingUser>(
                              initialValue: selectedUser,
                              decoration: const InputDecoration(labelText: '账号选择'),
                              items: visibleUsers
                                  .map(
                            ...
                            if (selectedUser?.requiresPassword ?? false) ...[
                              const SizedBox(height: 16),
                              TextField(
                  ...
                  child: Column(
                    children: [
                      MeetingTopStatusBar(
                        ...
                        onToggleNetwork: () => controller.recheckNetworkStatus(),
...
class _GlassChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final chip = Container(
      ...
      child: Text(
        '$label：$value',
        style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: palette.textOnChrome),
      ),
    );
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            color: active ? palette.accent : Colors.white.withValues(alpha: 0.32),
          ),
        ),
        if (!isLast)
          Container(
