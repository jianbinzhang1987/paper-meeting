...
class InfoPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final signedUsers = controller.session.users.where((user) => user.signedIn).length;
    ...
    return Column(
      ...
      children: [
        ...
        if (controller.bootstrapError != null) ...[
          ...
          _statusBanner(context, controller.bootstrapError!, icon: Icons.error_outline_rounded),
        ],
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Wrap(
                            ...
                            runSpacing: 10,
                            children: [
                              MeetingStatusPill(label: '会议室：${controller.config.roomName}', icon: Icons.meeting_room_outlined),
                              MeetingStatusPill(label: '座位：${controller.config.seatName}', icon: Icons.event_seat_outlined),
                              MeetingStatusPill(
                                label: '议题数：${controller.session.topics.length}',
                                icon: Icons.topic_outlined,
                                tone: MeetingStatusTone.accent,
                              ),
                              MeetingStatusPill(
                                label: '文稿数：${controller.session.documents.length}',
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                Expanded(
                                  child: Text('会议议题', style: Theme.of(context).textTheme.titleLarge),
                                ),
                                Text('当前会议流程', style: Theme.of(context).textTheme.bodySmall),
                              ],
                            ),
                            ...
                            Expanded(
                              child: ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  final topic = controller.session.topics[index];
                                  final selected = controller.selectedTopicId == topic.id;
                                  final relatedCount =
                                      controller.session.documents.where((item) => item.topicId == topic.id).length;
                                  return MeetingSelectableCard(
                                    onTap: () => controller.selectTopic(topic.id),
                                    selected: selected,
                                      ...
                                      children: [
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            border: selected ? null : Border.all(color: palette.panelBorder),
                                          ),
                                        ...
                                        MeetingStatusPill(
                                          ...
                                          tone: selected ? MeetingStatusTone.accent : MeetingStatusTone.neutral,
                                        ),
                                      ],
                                    ),
                                  );
              ...
              Expanded(
                ...
                child: MeetingSurfaceCard(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('参会人员', style: Theme.of(context).textTheme.titleLarge),
                      const SizedBox(height: 8),
                      Row(
                        children: [
                          Expanded(child: _statBlock(context, '应到', '$totalUsers')),
                          const SizedBox(width: 10),
                          Expanded(child: _statBlock(context, '签到', '$signedUsers')),
                          const SizedBox(width: 10),
                          Expanded(child: _statBlock(context, '未签到', '$unsignedUsers')),
                        ],
                      ),
                                      ...
                                      children: [
                                        ...
                                        Wrap(
                                          ...
                                          children: [
                                            ...
                                            MeetingStatusPill(label: user.seatName, icon: Icons.event_seat_outlined),
                                            MeetingStatusPill(
                                              label: user.signedIn ? '已签到' : '未签到',
                                              tone: user.signedIn ? MeetingStatusTone.accent : MeetingStatusTone.warning,
                                            ),
                                          ],
                                        ),
                                      ],
...
class _DocumentPanelState extends State<DocumentPanel> {
  ...
  void _preparePdfContext(AppController controller, MeetingDocument? document) {
    final resolvedUri = document == null ? null : _resolveDocumentUri(controller, document);
    final resolvedUrl = resolvedUri?.toString();
    if (_activeDocumentId == document?.id && _activeDocumentUrl == resolvedUrl) {
      return;
    }
    ...
    _pdfError = null;
    _pdfViewerController = document != null && _isPdfDocument(document) && resolvedUri != null ? PdfViewerController() : null;
  }
  ...
  Uri? _resolveDocumentUri(AppController controller, MeetingDocument document) {
    ...
    return Uri.http('${controller.config.serverIp}:${controller.config.port}', normalizedPath);
  }
        ...
        ...controller.notes.map(
          (item) => MeetingInsetBox(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(item.content, style: Theme.of(context).textTheme.titleSmall),
                const SizedBox(height: 6),
                Text('${item.createdBy} · 第 ${item.page} 页', style: Theme.of(context).textTheme.bodySmall),
              ],
            ),
          ),
        ),
  ...
  Widget _bookmarksTab(BuildContext context, AppController controller) {
    return ListView(
      children: [
        TextField(controller: bookmarkController, decoration: const InputDecoration(labelText: '书签名称')),
        const SizedBox(height: 10),
        MeetingActionButton(
  ...
  Widget _syncTab(BuildContext context, AppController controller) {
    final filteredLogs = logFilter == null
        ...
        : controller.messageLog.where((item) => item.type == logFilter).toList();
    return ListView(
      children: [
        MeetingActionButton(
        ...
        const SizedBox(height: 10),
        MeetingActionButton(
          onPressed: controller.followSync ? controller.leaveSync : controller.returnToSync,
          icon: Icon(controller.followSync ? Icons.exit_to_app_outlined : Icons.reply_all_outlined, size: 18),
            ...
            Expanded(
              child: DropdownButtonFormField<SyncMessageType?>(
                ...
                items: <DropdownMenuItem<SyncMessageType?>>[
                  const DropdownMenuItem<SyncMessageType?>(value: null, child: Text('全部')),
        ...
        ...filteredLogs.take(8).map(
          (item) => MeetingInsetBox(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(item.type.code, style: Theme.of(context).textTheme.titleSmall),
                const SizedBox(height: 6),
                Text(_formatTime(item.timestamp), style: Theme.of(context).textTheme.bodySmall),
              ],
            ),
          ),
        ),
            ...
            child: PdfViewer.uri(
              ...
              params: PdfViewerParams(
                ...
                onPageChanged: (pageNumber) {
                  if (pageNumber != null && controller.currentPage != pageNumber) {
                ...
                pagePaintCallbacks: controller.session.watermarkEnabled
                    ? <PdfViewerPagePaintCallback>[
                        (canvas, pageRect, page) {
                          final watermark = '${controller.currentUser?.name ?? '会议终端'}  ${_formatTime(DateTime.now())}';
                ...
                onDocumentLoadFinished: (documentRef, succeeded) {
                  ...
                  final error = succeeded ? null : documentRef.resolveListenable().error;
                  setState(() {
                    _pdfError = error?.toString();
                  });
                },
                loadingBannerBuilder: (context, bytesDownloaded, totalBytes) {
                  final progress = totalBytes == null || totalBytes == 0 ? null : bytesDownloaded / totalBytes;
                  return Align(
              ...
              SizedBox(
                ...
                child: MeetingSurfaceCard(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('议题与文稿', style: Theme.of(context).textTheme.titleLarge),
                      ...
                      Expanded(
                        child: controller.currentTopicDocuments.isEmpty
                            ? _panelStateCard(
                                ...
                                compact: true,
                                actionLabel: '刷新文稿',
                                onAction: controller.refreshingDocuments ? null : controller.refreshDocuments,
                              )
                            : ListView.separated(
                                itemCount: controller.currentTopicDocuments.length,
                                separatorBuilder: (_, __) => const SizedBox(height: 10),
                                itemBuilder: (context, index) {
                                  final doc = controller.currentTopicDocuments[index];
                                  final selected = controller.selectedDocumentId == doc.id;
                                  return MeetingSelectableCard(
                                    onTap: () => controller.selectDocument(doc.id),
                                    selected: selected,
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        ...
                                        const SizedBox(height: 6),
                                        Text('${doc.pageCount} 页', style: Theme.of(context).textTheme.bodySmall),
                                      ],
                                    ),
                                  );
                                },
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(currentDocument?.title ?? '暂无文稿', style: Theme.of(context).textTheme.headlineSmall),
                                const SizedBox(height: 8),
                                Wrap(
                                  spacing: 10,
                                  runSpacing: 10,
                                  children: [
                                    ...
                                    MeetingStatusPill(
                                      label: controller.followSync ? '正在跟随同屏' : '自由阅读模式',
                                      tone: controller.followSync ? MeetingStatusTone.accent : MeetingStatusTone.warning,
                                      icon: controller.followSync ? Icons.cast_connected_outlined : Icons.chrome_reader_mode_outlined,
                                    ),
                                    MeetingStatusPill(
                                      label: '角色：${controller.currentSyncRoleLabel}',
                          ...
                          const SizedBox(width: 12),
                          FilledButton.icon(
                            onPressed: currentDocument == null ? null : controller.requestSync,
                        ...
                        if (controller.preloading) ...[
                          const SizedBox(height: 12),
                          LinearProgressIndicator(value: controller.preloadProgress),
                        ],
                        const SizedBox(height: 16),
                        Expanded(
                          child: Stack(
                                ...
                                child: currentDocument == null
                                    ...
                                    : Column(
                                        ...
                                        children: [
                                          ...
                                          if ((currentDocument.fileType ?? '').isNotEmpty ||
                                              (currentDocument.fileUrl ?? '').isNotEmpty) ...[
                                            const SizedBox(height: 10),
                                            Text(
                                          ...
                                          if (_pdfError != null) ...[
                                            const SizedBox(height: 10),
                                            _statusBanner(context, _pdfError!, icon: Icons.warning_amber_rounded),
                                          ],
                                              ...
                                              child: ConstrainedBox(
                                                ...
                                                child: AspectRatio(
                                                  ...
                                                  child: Container(
                                                    ...
                                                    child: _buildDocumentViewport(context, controller, currentDocument),
                                                  ),
                                                ),
                                              ),
                                            ),
                                    ...
                                    children: [
                                      IconButton(
                                        onPressed: currentDocument == null
                                            ? null
                                            : () => _goToDocumentPage(controller, controller.currentPage - 1),
                                        icon: const Icon(Icons.chevron_left),
                                      ),
                                      SizedBox(width: 56, child: Text('${controller.currentPage}', textAlign: TextAlign.center)),
                                      Text('/ $pageCount', style: Theme.of(context).textTheme.bodySmall),
                                      const SizedBox(width: 10),
                                      Expanded(
                                        child: Slider(
                                          ...
                                          divisions: currentDocument != null && pageCount > 1
                                              ? pageCount - 1
                                              : 1,
                                          onChanged: currentDocument == null
                                              ? null
                                              : (value) => _goToDocumentPage(controller, value.round()),
                                        ),
                                      ),
                                      IconButton(
                                        onPressed: currentDocument == null
                                      ...
                                      IconButton(
                                        onPressed: currentDocument == null ? null : controller.toggleAnnotation,
                                        icon: Icon(controller.annotationEnabled ? Icons.draw_rounded : Icons.draw_outlined),
                                      ),
                                      IconButton(
                                        onPressed: controller.refreshingDocuments ? null : controller.refreshDocuments,
              ...
              SizedBox(
                ...
                child: MeetingSurfaceCard(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('辅助面板', style: Theme.of(context).textTheme.titleLarge),
                      const SizedBox(height: 14),
                        Container(
                          decoration: BoxDecoration(
                            color: const Color(0xFFF7FAFD),
...
class _MediaPanelState extends State<MediaPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    final allItems = controller.session.documents.where((item) {
      ...
      final isVideo = type.contains('video') || type.contains('mp4') || type.contains('m3u8');
      return widget.videoMode ? isVideo : !isVideo;
    }).toList();
    final categories = <String>{
      '全部',
      ...allItems.map((item) => (item.fileType ?? '').trim()).where((item) => item.isNotEmpty),
    }.toList();
    ...
    final items = allItems.where((item) {
      final matchesType = selectedType == '全部' || (item.fileType ?? '').trim() == selectedType;
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _header(context, widget.videoMode ? '会议视频' : '公共资料库', widget.videoMode ? '支持同步演示。' : '提供长期公共资料查阅。'),
        if (controller.bootstrapError != null) ...[
          const SizedBox(height: 12),
          _statusBanner(context, controller.bootstrapError!, icon: Icons.error_outline_rounded),
        ],
              ...
              SizedBox(
                ...
                child: DropdownButtonFormField<String>(
                  ...
                  items: categories
                      ...
                      .toList(),
                  onChanged: (value) => setState(() => selectedType = value ?? '全部'),
                ),
              ),
            ],
          ),
        ...
        Expanded(
          child: items.isEmpty
              ? _panelStateCard(
                  context,
                  icon: widget.videoMode ? Icons.play_circle_outline : Icons.folder_zip_outlined,
                  title: widget.videoMode ? '暂无会议视频' : '暂无公共资料',
                  message: widget.videoMode ? '当前会议还没有配置视频资源。' : '当前筛选条件下没有可查看资料。',
                )
              : GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final resourceUri = _resolveResourceUri(controller, item.fileUrl);
                    final cached = controller.isResourceCached(item.fileUrl);
                    final recent = controller.recentDocumentIds.contains(item.id);
                    return MeetingSurfaceCard(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Icon(widget.videoMode ? Icons.play_circle_outline : Icons.folder_zip_outlined, size: 36, color: Theme.of(context).colorScheme.primary),
                          const SizedBox(height: 12),
                          Text(item.title, style: Theme.of(context).textTheme.titleMedium),
                          if ((item.fileType ?? '').isNotEmpty) ...[
                            const SizedBox(height: 6),
                            Text(item.fileType!, style: Theme.of(context).textTheme.bodySmall),
                          ],
                          const SizedBox(height: 8),
                          Wrap(
                            spacing: 8,
                            runSpacing: 8,
                            children: [
                              if (recent) const MeetingStatusPill(label: '最近访问', tone: MeetingStatusTone.accent),
                              if (cached) const MeetingStatusPill(label: '已缓存', tone: MeetingStatusTone.success),
                            ],
                          ),
                          const Spacer(),
                          Wrap(
                            ...
                            children: [
                              if (widget.videoMode)
                                MeetingActionButton(
                                  onPressed: controller.playVideoSync,
                              ...
                              MeetingActionButton(
                                onPressed: resourceUri == null
                                    ? null
                                    : () async {
                                        controller.rememberRecentResource(item.id);
                                        final sourceUrl = _resolveResourceSourceUrl(controller, item.fileUrl);
                                        if (sourceUrl != null) {
                                          await controller.rememberCachedResource(sourceUrl);
                                        }
                                        final effectiveUri = _resolveResourceUri(controller, item.fileUrl) ?? resourceUri;
                                        await launchUrl(effectiveUri, mode: LaunchMode.externalApplication);
                                      },
                                icon: Icon(widget.videoMode ? Icons.open_in_new_rounded : Icons.download_outlined, size: 18),
                                label: widget.videoMode ? '打开视频' : '打开资料',
                              ),
                            ],
                          ),
                        ],
...
class VotePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalVotes = vote.options.fold<int>(0, (sum, item) => sum + item.count);
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            Expanded(child: _header(context, '投票表决', '支持实名或匿名表决，秘书可控制开始、结束、重置和发布结果。')),
            const SizedBox(width: 16),
            OutlinedButton.icon(
              onPressed: controller.refreshingVotes ? null : controller.refreshVotes,
              icon: const Icon(Icons.refresh_outlined),
        ...
        Expanded(
          child: !hasActiveVote
              ? _panelStateCard(
                  ...
                  actionLabel: '刷新表决',
                  onAction: controller.refreshingVotes ? null : controller.refreshVotes,
                )
              : Row(
                  children: [
                    Expanded(
                      child: MeetingSurfaceCard(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(vote.title, style: Theme.of(context).textTheme.headlineSmall),
                            const SizedBox(height: 12),
                            ...
                            Wrap(
                              ...
                              runSpacing: 12,
                              children: [
                                MeetingStatusPill(label: '状态：${vote.stage.label}'),
                                MeetingStatusPill(label: vote.anonymous ? '匿名表决' : '实名表决', tone: MeetingStatusTone.accent),
                                MeetingStatusPill(label: '已投人数：${vote.votedUserIds.length}/${controller.session.users.length}'),
                              ],
                            ),
                            const SizedBox(height: 20),
                            ...vote.options.map(
                              (option) => Padding(
                                ...
                                child: SizedBox(
                                  ...
                                  child: MeetingActionButton(
                                    onPressed: vote.stage == VoteStage.active && !hasVoted
                            ...
                            if (hasVoted) Text('当前账号已提交表决。', style: Theme.of(context).textTheme.bodySmall),
                          ],
                        ),
                      ),
                    ),
                    ...
                    Expanded(
                      child: MeetingSurfaceCard(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('实时统计', style: Theme.of(context).textTheme.titleLarge),
                            const SizedBox(height: 16),
                            ...vote.options.map(
                              (option) {
                                final percent = totalVotes == 0 ? 0.0 : option.count / totalVotes;
                                return Padding(
        ...
        Expanded(
          child: MeetingSurfaceCard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('请在下方签字区域完成电子签名。', style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 16),
                Expanded(
                  child: RepaintBoundary(
                    ...
                    child: SignaturePad(
                      ...
                      onPanStart: (details) => controller.addSignaturePoint(details.localPosition),
                      onPanUpdate: (details) => controller.addSignaturePoint(details.localPosition),
                ...
                if (submitMessage != null) ...[
                  const SizedBox(height: 12),
                  Text(submitMessage!, style: Theme.of(context).textTheme.bodySmall),
                ],
                const SizedBox(height: 16),
                Row(
                  children: [
                    FilledButton(
                      onPressed: controller.submittingSignature
                          ...
                          : () async {
                              try {
                                ...
                                final message = await controller.submitSignature(pngBytes);
                                if (!mounted) return;
                                setState(() {
                                  submitMessage = message == null ? '签名已提交' : '签名文件：$message';
                                });
                              } catch (error) {
                                if (!mounted) return;
                                setState(() {
                      ...
                      child: Text(controller.submittingSignature ? '提交中...' : '提交签名'),
                    ),
                    const SizedBox(width: 12),
                    OutlinedButton(
              ...
              Expanded(
                child: MeetingSurfaceCard(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('发起服务请求', style: Theme.of(context).textTheme.titleLarge),
                      const SizedBox(height: 16),
                      ...
                      FilledButton(
                        onPressed: () {
                          controller.addServiceRequest(
                            category: category,
                            detail: detailController.text.trim().isEmpty ? '无补充说明' : detailController.text.trim(),
                          );
              ...
              Expanded(
                ...
                child: MeetingSurfaceCard(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('服务回执', style: Theme.of(context).textTheme.titleLarge),
                      const SizedBox(height: 16),
                      Expanded(
                        child: controller.session.serviceRequests.isEmpty
                            ...
                            : ListView.separated(
                                itemCount: controller.session.serviceRequests.length,
                                separatorBuilder: (_, __) => const SizedBox(height: 12),
                                itemBuilder: (context, index) {
                                  final item = controller.session.serviceRequests[index];
                                  return MeetingInsetBox(
                                    padding: EdgeInsets.zero,
                                    child: ListTile(
                                      title: Text('${item.category} · ${item.requesterName}'),
                                      subtitle: Text('${item.seatName}｜${item.detail}'),
                                      trailing: MeetingStatusPill(
                                        label: item.status.label,
                                        tone: item.status == ServiceStatus.completed
                                            ? MeetingStatusTone.success
                                            : item.status == ServiceStatus.canceled
                                                ? MeetingStatusTone.danger
                                            : item.status == ServiceStatus.processing
                                                ? MeetingStatusTone.warning
                                                : MeetingStatusTone.neutral,
                                      ),
                                    ),
      ...
      children: [
        Row(
          ...
          children: [
            ...
            const SizedBox(width: 16),
            OutlinedButton.icon(
              onPressed: controller.refreshingNotices ? null : controller.refreshNotices,
              icon: const Icon(Icons.refresh_outlined),
        ...
        Expanded(
          child: notice == null
              ? _panelStateCard(
                  ...
                  actionLabel: '刷新通知',
                  onAction: controller.refreshingNotices ? null : controller.refreshNotices,
                )
              : MeetingSurfaceCard(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(_formatTime(notice.createdAt), style: Theme.of(context).textTheme.bodySmall),
                      const SizedBox(height: 12),
                      Wrap(
                        spacing: 10,
                        runSpacing: 10,
                        children: [
                          MeetingStatusPill(
                            label: controller.isNoticeRead(notice.id) ? '已读' : '未读',
                            tone: controller.isNoticeRead(notice.id) ? MeetingStatusTone.success : MeetingStatusTone.warning,
                          ),
                          MeetingStatusPill(
                            label: controller.isNoticePinned(notice.id) ? '已置顶' : '普通',
                            tone: controller.isNoticePinned(notice.id) ? MeetingStatusTone.accent : MeetingStatusTone.neutral,
                          ),
                        ],
                      ),
                      const SizedBox(height: 20),
                      ...
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 12),
                          MeetingActionButton(
                            onPressed: () => controller.toggleNoticePinned(notice.id),
                            label: controller.isNoticePinned(notice.id) ? '取消置顶' : '置顶通知',
                          ),
                          const SizedBox(width: 12),
                          MeetingActionButton(
                            onPressed: () {
...
class _TimerPanelState extends State<TimerPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _header(context, '个人计时器', '支持正计时与倒计时，适用于发言控制。'),
        const SizedBox(height: 16),
        Expanded(
          child: MeetingSurfaceCard(
            ...
            child: Column(
              children: [
                const SizedBox(height: 16),
                Text(_formatDuration(timerState.remainingSeconds), style: Theme.of(context).textTheme.displayMedium),
                ...
                Wrap(
                  ...
                  children: [
                    MeetingActionButton(
                      onPressed: () {
                        controller.setTimer(minutes: minuteValue.round(), countDown: countDown);
...
class _SecretaryPanelState extends State<SecretaryPanel> {
  ...
  Widget build(BuildContext context) {
    final controller = MeetingAppScope.of(context);
    final palette = context.meetingPalette;
    final logoutCandidates = controller.session.users.where((item) => item.id != controller.currentUser?.id).toList();
    final targetStillExists = logoutCandidates.any((item) => item.id == forceLogoutTarget?.id);
    if (!targetStillExists) {
      forceLogoutTarget = logoutCandidates.isNotEmpty ? logoutCandidates.first : null;
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
                  ...
                  children: [
                    MeetingSurfaceCard(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('表决控制中心', style: Theme.of(context).textTheme.titleLarge),
                          const SizedBox(height: 10),
                          Wrap(
                            spacing: 10,
                            runSpacing: 10,
                            children: [
                              MeetingStatusPill(label: '当前议题：${controller.session.voteItem.title}', icon: Icons.how_to_vote_outlined),
                              MeetingStatusPill(
                                label: '状态：${controller.session.voteItem.stage.label}',
                                tone: controller.session.voteItem.stage == VoteStage.active
                                    ? MeetingStatusTone.accent
                                    : controller.session.voteItem.stage == VoteStage.finished
                                        ? MeetingStatusTone.warning
                                        : MeetingStatusTone.neutral,
                              ),
                            ],
                    ...
                    MeetingSurfaceCard(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('同屏调度台', style: Theme.of(context).textTheme.titleLarge),
                          const SizedBox(height: 16),
                                ...
                                children: [
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text('${item.requesterName} · ${item.documentTitle}', style: Theme.of(context).textTheme.titleMedium),
                                        const SizedBox(height: 6),
                                        Text('状态：${item.status.label}', style: Theme.of(context).textTheme.bodySmall),
                                      ],
                                    ),
                                  ),
                                  if (item.status == SyncRequestStatus.pending)
                                    Wrap(
                                      ...
                                      children: [
                                        MeetingActionButton(
                                          onPressed: () => controller.approveSync(item.id),
                                          tone: MeetingActionTone.accent,
                                          label: '同意',
                                        ),
                                        MeetingActionButton(
                                          onPressed: () => controller.rejectSync(item.id),
                                          label: '拒绝',
                                        ),
                                      ],
                                    ),
                    ...
                    MeetingSurfaceCard(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('实时消息记录', style: Theme.of(context).textTheme.titleLarge),
                          const SizedBox(height: 12),
                          ...controller.messageLog.take(8).map(
                            (item) => MeetingInsetBox(
                              ...
                              child: ListTile(
                                ...
                                subtitle: Text('${_formatTime(item.timestamp)} · ${item.payload}'),
                              ),
                            ),
                          ),
                        ],
                  ...
                  children: [
                    MeetingSurfaceCard(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('呼叫服务后台处理', style: Theme.of(context).textTheme.titleLarge),
                          const SizedBox(height: 16),
                                ...
                                children: [
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text('${item.requesterName} · ${item.category}', style: Theme.of(context).textTheme.titleMedium),
                                        const SizedBox(height: 6),
                                        Text('${item.seatName}｜${item.detail}', style: Theme.of(context).textTheme.bodySmall),
                                      ],
                                    ),
                                  ),
                                  const SizedBox(width: 12),
                                  Container(
                                    ...
                                    child: DropdownButton<ServiceStatus>(
                                      ...
                                      onChanged: (status) {
                                        if (status != null) {
                                          controller.updateServiceStatus(item.id, status);
                                        }
                                      },
                                    ),
                                  ),
                    ...
                    MeetingSurfaceCard(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('全局窗口强制控制中心', style: Theme.of(context).textTheme.titleLarge),
                          const SizedBox(height: 12),
                          TextField(
                          ...
                          TextField(
                            ...
                            decoration: const InputDecoration(labelText: '广播内容'),
                            maxLines: 3,
                          ),
                          if (logoutCandidates.isNotEmpty) ...[
                            ...
                            DropdownButtonFormField<MeetingUser>(
                              ...
                              decoration: const InputDecoration(labelText: '强制注销目标'),
                              items: logoutCandidates
                                  .map(
                                    (user) => DropdownMenuItem<MeetingUser>(
                                      value: user,
                                      child: Text('${user.name} · ${user.role.label} · ${user.seatName}'),
                                    ),
                                  )
                                  .toList(),
                              onChanged: (value) => setState(() => forceLogoutTarget = value),
                            ),
                          ],
                          const SizedBox(height: 14),
                          Wrap(
                            ...
                            children: [
                              MeetingActionButton(
                                onPressed: () => controller.broadcastNotice(titleController.text, contentController.text),
                                tone: MeetingActionTone.primary,
                                label: '发送广播消息',
                              ),
                              MeetingActionButton(
                              ...
                              MeetingActionButton(
                                onPressed: forceLogoutTarget == null
                                    ? null
                                    : () => controller.forceLogoutUser(forceLogoutTarget!),
